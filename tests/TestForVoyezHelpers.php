...
class TestForVoyezHelpers extends WP_UnitTestCase
{
	...
	public function test_forvoyez_count_incomplete_images()
	{
		...
		$this->assertEquals(
			3,
			$incomplete_count,
			'Incorrect count of incomplete images',
		);
	...
	public function test_forvoyez_get_api_key()
	{
		...
		$this->assertEquals(
			'filtered_test_api_key',
			$filtered_api_key,
			'API key filter not applied correctly',
		);
	...
	public function test_forvoyez_sanitize_api_key()
	{
		...
		$this->assertEquals(
			$valid_jwt,
			$sanitized_jwt,
			'Valid JWT not sanitized correctly',
		);
		...
		$this->assertInstanceOf(
			WP_Error::class,
			$result,
			'Invalid JWT not detected',
		);
		$this->assertEquals(
			'invalid_api_key',
			$result->get_error_code(),
			'Incorrect error code for invalid JWT',
		);
		...
		$this->assertInstanceOf(
			WP_Error::class,
			$expired_verification_result,
			'Expired JWT not detected',
		);
		$this->assertEquals(
			'expired_token',
			$expired_verification_result->get_error_code(),
			'Incorrect error code for expired JWT',
		);
	}
