...
class TestForVoyezImageRenderer extends WP_UnitTestCase
{
	...
	public function test_is_metadata_complete()
	{
		...
		$this->assertTrue(
			$complete,
			'Metadata should be complete when all fields are filled',
		);
		...
		$this->assertFalse(
			$incomplete_alt,
			'Metadata should be incomplete when alt text is missing',
		);
		...
		$this->assertFalse(
			$incomplete_title,
			'Metadata should be incomplete when title is missing',
		);
		...
		$this->assertFalse(
			$incomplete_caption,
			'Metadata should be incomplete when caption is missing',
		);
		...
		$this->assertFalse(
			$all_empty,
			'Metadata should be incomplete when all fields are empty',
		);
	}
	...
	public function test_render_metadata_icons()
	{
		...
		$this->assertStringContainsString(
			'hidden alt-missing',
			$output,
			'Alt icon should be hidden when alt text is present',
		);
		$this->assertStringContainsString(
			'hidden title-missing',
			$output,
			'Title icon should be hidden when title is present',
		);
		$this->assertStringContainsString(
			'hidden caption-missing',
			$output,
			'Caption icon should be hidden when caption is present',
		);
		$this->assertStringNotContainsString(
			'hidden all-complete',
			$output,
			'All complete icon should be visible when all metadata is present',
		);
		...
		$this->assertStringNotContainsString(
			'hidden alt-missing',
			$output,
			'Alt icon should be visible when alt text is missing',
		);
		$this->assertStringNotContainsString(
			'hidden title-missing',
			$output,
			'Title icon should be visible when title is missing',
		);
		$this->assertStringNotContainsString(
			'hidden caption-missing',
			$output,
			'Caption icon should be visible when caption is missing',
		);
		$this->assertStringContainsString(
			'hidden all-complete',
			$output,
			'All complete icon should be hidden when metadata is incomplete',
		);
	}
