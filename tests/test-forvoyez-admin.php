...
class Test_Forvoyez_Admin extends WP_UnitTestCase {
	...
	public function test_get_image_counts() {
		...
		$this->factory->attachment->create_many( 5, array( 'post_mime_type' => 'image/webp' ) );

		// Create an image without alt text
		$attachment_id = $this->factory->attachment->create( array( 'post_mime_type' => 'image/webp' ) );
	...
	public function test_get_image_ids() {
		// Create test images
		$normal_image = $this->factory->attachment->create( array( 'post_mime_type' => 'image/webp' ) );
		update_post_meta( $normal_image, '_wp_attachment_image_alt', 'Normal Alt Text' );

		$missing_alt_image = $this->factory->attachment->create( array( 'post_mime_type' => 'image/webp' ) );
