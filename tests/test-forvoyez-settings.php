...
class TestForVoyezSettings extends WP_UnitTestCase {
	...
	public function test_encrypt_decrypt() {
		...
		$encrypted    = $this->invokeMethod( $this->settings, 'encrypt', array( $original_key ) );
		$decrypted    = $this->invokeMethod( $this->settings, 'decrypt', array( $encrypted ) );
	...
	public function test_get_api_key_set() {
		...
		$encrypted = $this->invokeMethod( $this->settings, 'encrypt', array( $test_key ) );
