...
class TestPluginMain extends WP_UnitTestCase
{
    ...
    public function test_required_files_included()
    {
        $expected_components = [
            'function' => [
                ...
                'forvoyez_count_incomplete_images',
                'forvoyez_sanitize_api_key',
                'forvoyez_verify_jwt',
            ],
            'class' => [
                'Forvoyez_Admin',
    ...
    {
        $this->assertTrue(
            function_exists('forvoyez_init'),
            'Function forvoyez_init does not exist.'
        );
    }
    ...
    public function test_hooks_registered()
    {
        ...
        $this->assertNotFalse(
            has_action('plugins_loaded', 'forvoyez_init'),
            'forvoyez_init is not hooked to plugins_loaded'
        );

        $this->assertArrayHasKey(
            'forvoyez_activate',
            $wp_filter['activate_' . FORVOYEZ_PLUGIN_BASENAME][10],
            'Activation hook is not registered'
        );
        $this->assertArrayHasKey(
            'forvoyez_deactivate',
            $wp_filter['deactivate_' . FORVOYEZ_PLUGIN_BASENAME][10],
            'Deactivation hook is not registered'
        );
    }
    ...
    public function test_activation()
    {
        ...
        $this->assertTrue(
            get_option('forvoyez_plugin_activated'),
            'Plugin activation flag not set.'
        );
        $this->assertEquals(
            FORVOYEZ_VERSION,
            get_option('forvoyez_plugin_version'),
            'Plugin version not set correctly.'
        );
        $this->assertTrue(
            get_option('forvoyez_flush_rewrite_rules'),
            'Flush rewrite rules flag not set.'
        );

        // Check if new options are set with default values
        $this->assertEquals(
            '',
    ...
    public function test_deactivation()
    {
        ...
        $this->assertFalse(
            get_option('forvoyez_plugin_activated'),
            'Plugin activation flag not removed.'
        );
        $this->assertFalse(
            get_option('forvoyez_flush_rewrite_rules'),
            'Flush rewrite rules flag not removed.'
        );

        // Check if the new options are preserved after deactivation
        $this->assertEquals(
            'Test Context',
            get_option('forvoyez_context'),
            'Context should be preserved after deactivation'
        );
        $this->assertEquals(
            'fr',
            get_option('forvoyez_language'),
            'Language should be preserved after deactivation'
        );
    }
}