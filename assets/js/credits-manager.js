...
(function($) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    console.error('Failed to get credits information:', response.data.message);
                }
            },
    ...
    function updateCreditsDisplay(data) {
        ...
        if (creditCountElement && creditCountElement.length) {
            ...
            if (credits > 20) {
                creditCountElement.removeClass('bg-yellow-100 bg-red-100 text-yellow-800 text-red-800')
                    .addClass('bg-green-100 text-green-800');
            } else if (credits > 5) {
                creditCountElement.removeClass('bg-green-100 bg-red-100 text-green-800 text-red-800')
                    .addClass('bg-yellow-100 text-yellow-800');
            } else {
                creditCountElement.removeClass('bg-green-100 bg-yellow-100 text-green-800 text-yellow-800')
        ...
        if (creditsStatusElement && creditsStatusElement.length) {
            ...
            if (isSubscribed) {
                statusHtml = '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Actif</span>';
                if (planName) {
                    statusHtml += ' - ' + planName;
                }
            } else {
                statusHtml = '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Aucun abonnement actif</span>';
            }
    ...
    function updateGlobalCreditDisplay() {
        ...
        if (globalCreditElement.length) {
            ...
            if (credits > 20) {
                globalCreditElement.removeClass('bg-yellow-100 bg-red-100 text-yellow-800 text-red-800')
                    .addClass('bg-green-100 text-green-800');
            } else if (credits > 5) {
                globalCreditElement.removeClass('bg-green-100 bg-red-100 text-green-800 text-red-800')
                    .addClass('bg-yellow-100 text-yellow-800');
            } else {
                globalCreditElement.removeClass('bg-green-100 bg-yellow-100 text-green-800 text-yellow-800')
                    .addClass('bg-red-100 text-red-800');
            }
        }
    }
    ...
    function createCreditsWidget() {
        ...
        if ($('.forvoyez-credits-widget').length === 0) {
            const widget = $(`
                <div class="forvoyez-credits-widget fixed bottom-4 right-4 bg-white p-3 rounded-lg shadow-lg border border-gray-200 z-50">
