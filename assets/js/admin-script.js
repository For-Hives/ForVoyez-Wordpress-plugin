;/* global forvoyezData, jQuery */(function ($) {
	'use strict'
	...
	$( document ).ready(
		...
		$( '.forvoyez-save-additional-settings' ).on(
            'click',
            function (e) {
			e.preventDefault()
			...
			$.ajax(
                {
					...
					success: function (response) {
						if (response.success) {
							...
							updateMediaLibraryFields( imageId, response.data.metadata );
						} else {
							showNotification( 'Failed to analyze image: ' + response.data.message, 'error' );
						}
					},
					error: function () {
						showNotification( 'An error occurred while analyzing the image', 'error' );
					},
					complete: function () {
						$button.prop( 'disabled', false ).text( 'Analyze with ForVoyez' );
					}
                });
            }
        );

        function updateMediaLibraryFields(imageId, metadata) {
            var $form = $( 'form[data-id="' + imageId + '"]' );
            if ($form.length) {
                $form.find( 'input[data-setting="alt"]' ).val( metadata.alt_text ).change();
                $form.find( 'input[data-setting="title"]' ).val( metadata.title ).change();
                $form.find( 'textarea[data-setting="caption"]' ).val( metadata.caption ).change();
            }
        }
	...
	$filterForm.find( 'input[type="checkbox"]' ).on(
        'change',
        function () {
		currentPage = 1
		loadImages( currentPage )
	...
	function analyzeBulkImages(imageIds) {
		...
		function updateProgress() {
			...
			showNotification(
				`Processing: ${progress}% complete. Successful: ${processedCount},
                Failed: ${failedCount}`,
				'info',
				0
			)
		...
		async function processAllImages() {
			...
			showNotification(
				`Analysis complete. Successful: ${processedCount},
                Failed: ${failedCount}`,
				'success',
				5000
			)
