...
class Forvoyez_Image_Processor {
    ...
    public function init() {
        ...
        add_action( 'wp_ajax_forvoyez_update_image_metadata', array( $this, 'ajax_update_image_metadata' ) );
        add_action( 'wp_ajax_forvoyez_load_more_images', array( $this, 'ajax_load_more_images' ) );
        add_action( 'wp_ajax_forvoyez_bulk_analyze_images', array( $this, 'ajax_bulk_analyze_images' ) );
        add_action( 'wp_ajax_forvoyez_analyze_single_image', array( $this, 'ajax_analyze_single_image' ) );
        add_action( 'wp_ajax_forvoyez_process_image_batch', array( $this, 'ajax_process_image_batch' ) );
    }
    ...
    public function ajax_update_image_metadata() {
        ...
        if ( !$image_id || empty( $metadata ) ) {
            wp_send_json_error( esc_html__( 'Invalid data', 'forvoyez-auto-alt-text-for-images' ) );
        }

        $this->update_image_meta( $image_id, $metadata );

        wp_send_json_success( esc_html__( 'Metadata updated successfully', 'forvoyez-auto-alt-text-for-images' ) );
    }
    ...
    public function ajax_analyze_image() {
        ...
        if ( !$image_id || !wp_attachment_is_image( $image_id ) ) {
            wp_send_json_error( esc_html__( 'Invalid image ID', 'forvoyez-auto-alt-text-for-images' ) );
        }
        ...
        if ( $result['success'] ) {
            wp_send_json_success(
                array(
                    'message'  => esc_html__( 'Analysis successful', 'forvoyez-auto-alt-text-for-images' ),
            ...
            wp_send_json_error(
                array(
                    'message' => esc_html( $result['error']['message'] ),
                    'code'    => $result['success'] ? null : ( $result['error']['code'] ?? esc_html__( 'unknown_error', 'forvoyez-auto-alt-text-for-images' ) ),
                )
            );
        }
    }
    ...
    public function ajax_load_more_images() {
        ...
        $per_page = isset( $_POST['per_page'] ) ? absint( $_POST['per_page'] ) : 21;
    ...
    public function ajax_bulk_analyze_images() {
        $this->verify_ajax_request();
        ...
        if ( empty( $image_ids ) ) {
            wp_send_json_error( esc_html__( 'No valid images selected', 'forvoyez-auto-alt-text-for-images' ) );
        }

        wp_send_json_success(
            array(
                'message'   => esc_html__( 'Processing started', 'forvoyez-auto-alt-text-for-images' ),
    ...
    public function ajax_analyze_single_image() {
        $this->verify_ajax_request();
    ...
    private function verify_ajax_request() {
        ...
        if ( !current_user_can( 'upload_files' ) ) {
            wp_send_json_error( esc_html__( 'Permission denied', 'forvoyez-auto-alt-text-for-images' ) );
        }
    }
}